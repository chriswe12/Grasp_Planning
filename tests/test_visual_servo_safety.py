...
def test_source_image_age_can_be_warning_only_for_transport_testing() -> None:
    config = replace(_supervisor().config, enforce_source_image_age=False)
    supervisor = VisualServoSafetySupervisor(config)
    supervisor.mark_ready()
    supervisor.arm(now_s=9.0)
    ...
    assert decision.state == VisualServoState.RUNNING
    assert not decision.terminal
