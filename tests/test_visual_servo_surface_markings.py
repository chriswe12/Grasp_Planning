...
def test_markings_are_deterministic_diverse_and_outside_target_corridor() -> None:
    ...
    dirt_count = sum(marking.kind == "dirt" for markings in first for marking in markings)
    active_count = sum(bool(markings) for markings in first)
    assert dirt_count >= 1.20 * active_count
    assert all(math.hypot(marking.x_m, marking.y_m) >= 0.075 for markings in first for marking in markings)
    assert sum(
        math.hypot(marking.x_m, marking.y_m) <= 0.18 for markings in first for marking in markings
...
def test_marking_sampler_supports_completely_clean_scenes_and_validates_ranges() -> None:
    ...
    assert sample_surface_markings(4, enabled=True, seed=1, environment_fraction=0.0) == ((), (), (), ())
