...
def test_reproducibility_diversity_and_bounds():
    ...
    assert .2 < sum(s['part_color_name']=='blue' for s in samples)/len(samples) < .3
    assert .73 < sum(s['wear_visible'] for s in samples)/len(samples) < .87
    for s in samples:
...
def test_rejects_invalid_profile():
    profile = load_profile(PROFILE)
    profile['light_intensity'] = [100., float('nan')]
