...
from grasp_planning.rl.zed_mini import (
    load_zed_profile, pack_zed_rgbd, offset_jacobian, reproject_intrinsics,
)
...
def test_invalid_depth_does_not_pollute_valid_surface():
    ...
    depth = torch.tensor([[[[.2], [float('inf')]], [[0.], [float('nan')]]]])
...
def test_tcp_offset_angular_velocity_sign():
    ...
    moved = offset_jacobian(jac, torch.tensor([[2., 0., 0.]]))
    torch.testing.assert_close(moved[0, :3, 0], torch.tensor([0., 2., 0.]))
