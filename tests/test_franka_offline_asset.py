...
def test_offline_asset_checks_identity_and_dependency_bytes(tmp_path):
    ...
    manifest = dict(source_url='https://example.test/robot.usda', entry='robot.usda',
                    files={name: hashlib.sha256((tmp_path / name).read_bytes()).hexdigest()
                           for name in ('robot.usda', 'mesh.usda')})
...
def test_offline_asset_rejects_path_escape(tmp_path):
    path = tmp_path / 'manifest.json'
    path.write_text(json.dumps(dict(source_url='robot', entry='../robot.usda',
                                    files={'../robot.usda': 'unused'})))
