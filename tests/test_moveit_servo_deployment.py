...
def test_aligned_moveit_servo_uses_speed_units_tcp_and_collision_checking() -> None:
    path = REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/iiwa7_y_gripper_moveit_servo.yaml"
    ...
    pdz_path = REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/iiwa7_pdz_gripper_moveit_servo.yaml"
    pdz_servo = yaml.safe_load(pdz_path.read_text(encoding="utf-8"))["/**/servo_node"]["ros__parameters"][
        "moveit_servo"
    ]
...
def test_aligned_launch_and_shell_wrapper_expose_opt_in_servo() -> None:
    launch = (REPO_ROOT / "ros2_ws/src/robot_integration_ros/launch/aligned_lbr_moveit.launch.py").read_text(
        encoding="utf-8"
    )
...
def test_single_arm_servo_receives_complete_robot_state() -> None:
    semantic = (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/iiwa7_y_gripper.srdf.xacro").read_text(
        encoding="utf-8"
    )
    legacy_controllers = yaml.safe_load(
        (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/single_lbr_controllers.yaml").read_text(encoding="utf-8")
    )

    assert '<passive_joint name="left_finger_joint"/>' in semantic
    pdz_semantic = (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/iiwa7_pdz_gripper.srdf.xacro").read_text(
        encoding="utf-8"
    )
    ...
    servo = yaml.safe_load(
        (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/iiwa7_y_gripper_moveit_servo.yaml").read_text(
            encoding="utf-8"
        )
    ...
    pdz_controllers = yaml.safe_load(
        (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/single_lbr_controllers_pdz_gripper.yaml").read_text(
            encoding="utf-8"
        )
    )
...
def test_single_arm_policy_uses_existing_lbr_force_broadcaster() -> None:
    deployment = yaml.safe_load((REPO_ROOT / "configs/visual_servo_real_d405.yaml").read_text(encoding="utf-8"))[
        "visual_servo"
    ]
    launch = (REPO_ROOT / "ros2_ws/src/robot_integration_ros/launch/aligned_lbr_moveit.launch.py").read_text(
        encoding="utf-8"
...
def test_dual_moveit_servo_has_one_collision_checked_route_per_arm() -> None:
    deployment = yaml.safe_load(
        (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/dual_iiwa7_pdz_gripper_moveit_servo.yaml").read_text(
            encoding="utf-8"
        )
    )
    ...
    launch = (REPO_ROOT / "ros2_ws/src/robot_integration_ros/launch/dual_aligned_lbr_moveit.launch.py").read_text(
        encoding="utf-8"
    )
...
def test_dual_hardware_uses_split_controller_managers_and_servo_outputs() -> None:
    hardware_servo = yaml.safe_load(
        (
            REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/dual_iiwa7_pdz_gripper_moveit_servo_hardware.yaml"
    ...
    controllers = yaml.safe_load(
        (REPO_ROOT / "ros2_ws/src/robot_integration_ros/config/dual_lbr_moveit_controllers_hardware.yaml").read_text(
            encoding="utf-8"
        )
