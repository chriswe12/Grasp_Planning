...
def test_quintic_stream_respects_speed_limit_and_stops_at_target() -> None:
    ...
    assert context.position_commands[-1] == 0.5
...
def test_large_move_is_not_silently_compressed_past_speed_limit() -> None:
    context = _TrackingContext()
    executor = TrajectoryExecutor(context, max_joint_speed_rad_s=0.5, final_settle_steps=2)
