...
def make_pencil_marks(seed=42, density=1.0, contrast=1.0):
    ...
    mats = []
    for i, value in enumerate([0.29, 0.34, 0.39, 0.45, 0.50]):
        rgb = tuple(
            a * (1 - contrast) + b * contrast for a, b in zip((0.68, 0.65, 0.59), (value, value * 0.99, value * 0.97))
        )
        mats.append(material(f"Pencil_Graphite_{i}", rgb, 0.84))
    vertices = []
    faces = []
    mat_ids = []
    strokes = 0
    ...
    def blocked(x, y):
        return any(abs(x - a) < w / 2 + 0.007 and abs(y - b) < h / 2 + 0.007 for a, b, w, h in tapes)

    def stroke(points, width=0.0007, shade=None):
        nonlocal strokes
        if shade is None:
            shade = rng.choices(range(5), weights=[1, 3, 4, 3, 2])[0]
        strokes += 1
        for i, ((x1, y1), (x2, y2)) in enumerate(zip(points, points[1:])):
            if blocked((x1 + x2) / 2, (y1 + y2) / 2):
                continue
            if max(abs(x1), abs(x2)) > 0.76 or max(abs(y1), abs(y2)) > 0.36:
                continue
            dx = x2 - x1
            dy = y2 - y1
            ...
            if rng.random() < 0.055:
                continue
            taper = max(0.38, math.sin(math.pi * (i + 0.5) / max(1, len(points) - 1)))
            half = width * 0.5 * rng.uniform(0.68, 1.25) * taper
            nx = -dy / length * half
            ny = dx / length * half
            k = len(vertices)
            vertices.extend(
            ...
            mat_ids.append(shade)

    def line(cx, cy, length, angle, width=0.0007, shade=None):
        pts = []
        for i in range(9):
            t = (i / 8 - 0.5) * length
            jitter = rng.uniform(-0.00035, 0.00035)
            pts.append(
                (
                    cx + t * math.cos(angle) - jitter * math.sin(angle),
                    cy + t * math.sin(angle) + jitter * math.cos(angle),
                )
            )
    ...
    for _ in range(round(140 * density)):
        cx = rng.gauss(0.34, 0.15)
        cy = rng.gauss(0.12, 0.115)
        angle = rng.gauss(-0.12, 0.34)
        for j in range(rng.choices([1, 2, 3], [5, 3, 1])[0]):
            line(
                cx,
                cy + j * rng.uniform(0.0018, 0.0035),
                rng.uniform(0.008, 0.040),
                angle,
                rng.uniform(0.00042, 0.00105),
            )
    # Hand-sketched fixture outline and offset retraces, slightly irregular.
    for offset, shade in [(0.0, 2), (0.0018, 4)]:
        x0, x1 = 0.17 + offset, 0.43 + offset
        y0, y1 = 0.075 + offset, 0.25 + offset
        for cx, cy, length, angle in [
            ((x0 + x1) / 2, y0, x1 - x0, 0),
            ((x0 + x1) / 2, y1, x1 - x0, 0),
            (x0, (y0 + y1) / 2, y1 - y0, math.pi / 2),
            (x1, (y0 + y1) / 2, y1 - y0, math.pi / 2),
        ]:
            line(cx, cy, length, angle, 0.00065, shade)
    for i in range(round(9 * density)):
        x = 0.18 + i * 0.027
        line(
            x, 0.16, rng.uniform(0.10, 0.175), math.pi / 2 + rng.uniform(-0.025, 0.025), rng.uniform(0.0004, 0.0007), 3
        )
    # Small cross-hatching / scrubbing clusters rather than a uniform noisy coating.
    for _ in range(round(11 * density)):
        cx = rng.uniform(0.06, 0.62)
        cy = rng.uniform(-0.16, 0.27)
        for j in range(rng.randint(4, 8)):
            line(
                cx + j * 0.002,
                cy + j * 0.0007,
                rng.uniform(0.006, 0.019),
                rng.uniform(0.5, 0.8),
                rng.uniform(0.00045, 0.0009),
                rng.choice([1, 2, 3]),
            )
    # A few curved scribble fragments: pencil slips, not printed text.
    for _ in range(round(8 * density)):
        cx = rng.uniform(0.10, 0.58)
        cy = rng.uniform(-0.15, 0.23)
        sx = rng.uniform(0.008, 0.018)
        sy = rng.uniform(0.002, 0.005)
        pts = [
            (cx + (i / 27 - 0.5) * sx * 3, cy + sy * math.sin(i / 27 * math.pi * 4) + rng.uniform(-0.0003, 0.0003))
            for i in range(28)
        ]
        stroke(pts, 0.00065, 3)
    # Sparse marks beyond the primary right-hand work area.
    for _ in range(round(20 * density)):
        line(
            rng.uniform(-0.55, -0.1),
            rng.uniform(-0.24, 0.24),
            rng.uniform(0.008, 0.029),
            rng.uniform(-0.4, 0.4),
            0.00055,
            4,
        )
    ...
    mesh.update()
    obj = bpy.data.objects.new("Handling_Scuff_PencilStrokes", mesh)
    col.objects.link(obj)
    obj.parent = parent
    for m in mats:
        mesh.materials.append(m)
    for face, index in zip(mesh.polygons, mat_ids):
    ...
    if text:
        updated = text.as_string().replace(
            "bpy.data.collections['Surface_Wear'].objects", "list(bpy.data.collections['Surface_Wear'].all_objects)"
        )
        text.clear()
        text.write(updated)
...
def main():
    ...
    if not 0.1 <= a.density <= 3 or not 0 <= a.contrast <= 1:
        raise ValueError("density: 0.1..3; contrast: 0..1")
    ...
    config = dict(
        ...
        contrast=a.contrast,
        stroke_count=count,
        collection="Pencil_Marks",
        usd_root="/Lab/Handling_Scuff_PencilMarks",
    )
    (out / "pencil_config.json").write_text(json.dumps(config, indent=2) + "\n")
    manifest = json.loads((out / "manifest.json").read_text())
    manifest["pencil_marks"] = config
    ...
    assert all(
        not p.HasAPI(UsdPhysics.CollisionAPI) and not p.HasAPI(UsdPhysics.RigidBodyAPI)
        for p in Usd.PrimRange(stage.GetPrimAtPath("/Lab/Handling_Scuff_PencilMarks"))
    )
    ...
    (out / "validation.json").write_text(
        json.dumps(
            dict(
                status="passed",
                earlier_packages_unchanged=True,
                collision_paths_unchanged=True,
                pencil_marks_render_only=True,
    ...
    if a.render:
        ...
        scene.render.filepath = str(out / "comparison_before_detail.png")
        bpy.ops.render.render(write_still=True)
