...
add_environment = module.add_environment
randomize_environment = module.randomize_environment
add_mock_props = module.add_mock_props
sample_prop_layout = module.sample_prop_layout
from randomize_video_lab import randomize


def main():
    folder = ROOT / "assets/scenes/video_lab"
    ...
    for p in collision:
        assert UsdGeom.Imageable(p).GetPurposeAttr().Get() == "guide"
    files, assets, unresolved = UsdUtils.ComputeAllDependencies(str(folder / "preview.usda"))
    assert not unresolved, unresolved
    for info in manifest["props"]:
        ps = Usd.Stage.Open(str(folder / info["asset"]))
        root = ps.GetDefaultPrim()
    ...
    d = manifest["dimensions"]["depth"]
    for seed in range(200):
        layout = sample_prop_layout(seed, manifest)
        assert layout == sample_prop_layout(seed, manifest)
        counts.add(len(layout))
        for i, p in enumerate(layout):
            ...
            assert all(math.hypot(x - a, y - b) >= r + c + 0.02 - 1e-9 for a, b, c in exclusions)
    ...
    add_environment(test, "/World/Env0/Lab", folder)
    add_environment(test, "/World/Env1/Lab", folder)
    ...
    b = randomize_environment(test, "/World/Env1/Lab", 23, folder)
    assert a != b
    ...
    for index, sample in [(0, a), (1, b)]:
        ...
        assert (
            UsdGeom.BBoxCache(Usd.TimeCode.Default(), ["guide"]).ComputeWorldBound(local_support).ComputeAlignedRange()
            == bounds
        )
    ...
    a = randomize(7)
    first = (
        bpy.data.materials["Warm_White_Partition"]
        .node_tree.nodes["Principled BSDF"]
        .inputs["Base Color"]
        .default_value[:]
    )
    randomize(23)
    assert randomize(7) == a
    assert (
        bpy.data.materials["Warm_White_Partition"]
        .node_tree.nodes["Principled BSDF"]
        .inputs["Base Color"]
    ...
    for seed in range(200):
        ...
        for i, p in enumerate(poses):
            ...
            assert all(math.hypot(p["x"] - x, p["y"] - y) >= p["radius"] + r + 0.02 - 1e-9 for x, y, r in exclusions)
    result = dict(
        status="passed",
        blender_layout_seeds_checked=200,
        usd_dependencies_resolved=True,
        static_colliders=2,
        props_checked=len(manifest["props"]),
        layout_seeds_checked=200,
        observed_prop_counts=sorted(counts),
        table_support_z_m=bounds.GetMax()[2],
        randomization_repeatable=True,
        per_environment_materials_independent=True,
        source_asset_unchanged=True,
        blender_version=bpy.app.version_string,
        isaac_simulation="Not run: NVIDIA driver unavailable in this session",
    )
