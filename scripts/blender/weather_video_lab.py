...
def author_weathering(base, seed, amount):
    ...
    fleck = np.clip((cloud - 0.25) / 0.65, 0, 1)
    ...
    def patch(cx, cy, sx, sy, angle=0):
        ...
        return np.exp(-((u / sx) ** 2 + (v / sy) ** 2) * 1.7) * (0.24 + 0.76 * fleck)
    ...
    for cx, cy, sx, sy, a, k in [
        (0.34, 0.12, 0.23, 0.12, 0.2, 0.33),
        (-0.46, 0.25, 0.18, 0.065, -0.1, 0.24),
        (0.53, -0.19, 0.12, 0.05, 0.3, 0.18),
        (-0.15, -0.23, 0.20, 0.055, -0.3, 0.12),
    ]:
        warm += k * patch(cx, cy, sx, sy, a)
    for cx, cy, sx, sy, a, k in [
        (0.35, 0.14, 0.13, 0.075, 0.1, 0.26),
        (-0.57, 0.24, 0.11, 0.06, 0.3, 0.22),
        (0.60, 0.26, 0.10, 0.05, -0.2, 0.20),
        (0.19, -0.28, 0.18, 0.025, -0.15, 0.11),
        (-0.37, -0.02, 0.12, 0.028, 0.6, 0.11),
    ]:
        dirt += k * patch(cx, cy, sx, sy, a)
    # Finger/cloth drag marks, clustered around work rather than equally spaced.
    for _ in range(34):
        cx = rng.normal(0.32, 0.13)
        cy = rng.normal(0.12, 0.08)
        dirt += rng.uniform(0.025, 0.075) * patch(
            cx, cy, rng.uniform(0.01, 0.055), rng.uniform(0.0007, 0.0035), rng.uniform(-0.3, 0.3)
        )
    # Edge and center-joint dirt is low amplitude and discontinuous.
    edge = np.exp(-(((y + 0.394) / 0.010) ** 2)) + np.exp(-(((y - 0.393) / 0.009) ** 2))
    seam = np.exp(-((x / 0.0028) ** 2))
    dirt += (edge * 0.038 + seam * 0.025) * fleck
    # Sparse tiny spots: not modeled debris and not baked object shadows.
    for _ in range(95):
        cx = rng.uniform(-0.75, 0.75)
        cy = rng.uniform(-0.36, 0.36)
        dirt += rng.uniform(0.025, 0.12) * patch(cx, cy, rng.uniform(0.0005, 0.0016), rng.uniform(0.0004, 0.0013))
    # Soft interrupted ring of handling/adhesive residue near the rear work area.
    r = np.sqrt(((x - 0.38) / 1.05) ** 2 + (y - 0.215) ** 2)
    ring = np.exp(-(((r - 0.032) / 0.0013) ** 2)) * np.clip((cloud - 0.35) * 2, 0, 1)
    warm += ring * 0.075
    dirt = np.clip(dirt, 0, 0.50) * amount
    warm = np.clip(warm, 0, 0.45) * amount
    # Keep overall laminate recognizable; add chromatic variation locally.
    aged = base * np.array([1.014, 1.001, 0.974], dtype=np.float32)
    color = aged * (1 - dirt[:, :, None])
    amber = np.array([0.36, 0.265, 0.145], dtype=np.float32)
    color = color * (1 - warm[:, :, None]) + amber * warm[:, :, None]
    rough = np.clip(0.38 + dirt * 0.55 + warm * 0.28 + (cloud - 0.5) * 0.035, 0.28, 0.65)
...
def layout_material(nt):
    ...
    nt.nodes["Table_Weathering_Strength"].color = (0.32, 0.19, 0.06)
...
def main():
    ...
    mat = bpy.data.materials["Table_Laminate"]
    nt = mat.node_tree
    albedo = nt.nodes["Laminate_Albedo"]
    clean_image = albedo.image
    rough = nt.nodes["Laminate_Roughness"]
    clean_rough = rough.image
    color, roughness = author_weathering(linear_rgb(clean_image), args.seed, args.amount)
    rgba = np.ones((*color.shape[:2], 4), dtype=np.float32)
    rgba[:, :, :3] = color
    ...
    albedo.image = dirty_image
    rough.image = rough_image
    original = nt.nodes.new("ShaderNodeTexImage")
    original.name = "Original_Laminate_Albedo"
    original.image = clean_image
    original_rough = nt.nodes.new("ShaderNodeTexImage")
    original_rough.name = "Original_Laminate_Roughness"
    original_rough.image = clean_rough
    ...
    for node, filename in [
        ("Laminate_Albedo", "laminate_weathered_albedo.png"),
        ("Laminate_Roughness", "laminate_weathered_roughness.png"),
    ]:
    ...
    manifest["appearance_variant"] = dict(
        name="warm_localized_table_weathering_v1",
        seed=args.seed,
        amount=args.amount,
        fallback="../video_lab/video_lab.blend",
        blender_strength_node="Table_Weathering_Strength",
    )
    ...
    if args.render:
        ...
        nt.nodes["Table_Weathering_Strength"].outputs[0].default_value = 0.0
        nt.update_tag()
        bpy.context.view_layer.update()
        scene.render.filepath = str(out / "comparison_original_detail.png")
        bpy.ops.render.render(write_still=True)
