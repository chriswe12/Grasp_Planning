...
def physics_material(stage):
    ...
    api.CreateStaticFrictionAttr(0.8)
...
def export_package(out, props, cfg):
    ...
    nt = bpy.data.materials["Table_Laminate"].node_tree
    ...
    material = physics_material(stage)
    collision_paths = []
    for prim in stage.Traverse():
        if prim.IsA(UsdGeom.Mesh) and (
            "Table_Support_Collision" in str(prim.GetPath()) or "Floor_Collision" in str(prim.GetPath())
        ):
    ...
    for root in props:
        ...
        prop_info.append(
            dict(
                ...
                mass_kg=root["mass_kg"],
                footprint_radius_m=root["footprint_radius_m"],
                preview_position=list(loc),
                preview_yaw_rad=rot.z,
            )
        )
    ...
    stage = Usd.Stage.CreateNew(str(out / "preview.usda"))
    ...
    for info in prop_info:
        obj = UsdGeom.Xform.Define(stage, "/World/Props/" + info["name"])
        obj.GetPrim().GetReferences().AddReference(info["asset"])
        obj.AddTranslateOp().Set(Gf.Vec3d(*info["preview_position"]))
        obj.AddRotateZOp().Set(info["preview_yaw_rad"] * 180 / 3.141592653589793)
    stage.GetRootLayer().Save()
    manifest = dict(
        schema_version=1,
        environment="environment.usdc",
        blender="video_lab.blend",
        root_prim="/Lab",
        up_axis="Z",
        meters_per_unit=1.0,
        table_top_z_m=0.0,
        room_floor_z_m=-cfg["table"]["height"],
        dimensions=cfg["table"],
        dimension_status=cfg["dimension_status"],
        collision_paths=collision_paths,
        props=prop_info,
        randomization=cfg["randomization"],
        limitations=[
            "Video-derived approximation, no metric calibration",
            "USD uses portable PreviewSurface PBR; procedural Blender micro-bump is not exported",
            "Prop collisions use per-mesh convex hulls; bores are visual, not insertion-accurate",
            "No robot or fixed training task is embedded",
        ],
    )
