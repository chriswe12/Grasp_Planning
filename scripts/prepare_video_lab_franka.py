...
p.add_argument('--base-position', type=float, nargs=3, default=(-.48, .10, .0))
a = p.parse_args()
out = a.output_dir.resolve(); out.mkdir(parents=True, exist_ok=True)
robot_asset = out/'robot/panda_instanceable.usd'
if not robot_asset.is_file(): raise FileNotFoundError(robot_asset)
...
joints = {f'panda_joint{i+1}': q for i, q in enumerate([0., -.785, 0., -2.356, 0., 1.571, .785])}
joints.update(panda_finger_joint1=.04, panda_finger_joint2=.04)

def frame(pos, rot):
    m = Gf.Matrix4d(1.)
    m.SetRotate(Gf.Quatd(rot)); m.SetTranslateOnly(Gf.Vec3d(pos))
    return m

# All rigid link transforms are siblings beneath the articulation root.
transforms = {'/World/Franka/panda_link0': Gf.Matrix4d(1.)}
...
while pending:
    ...
    for joint in list(pending):
        ...
        if str(body0[0]) not in transforms: continue
        name = joint.GetPrim().GetName(); q = joints.get(name, 0.)
    ...
    if not progress: raise RuntimeError('Disconnected Panda joint graph')
...
def camera(name, eye, target, focal=40.):
    ...
    return cam
camera('Overview', (1.65,-2.35,1.65), (-.04,.04,.24), 43.)
camera('WorkArea', (.94,-1.42,1.07), (-.05,.02,.23), 38.)
camera('RobotSide', (-1.3,-1.85,1.15), (-.12,.04,.27), 40.)
...
ros_to_usd=Gf.Matrix4d(1.); ros_to_usd.SetRotate(Gf.Rotation(Gf.Vec3d(1,0,0),180.))
...
cam.CreateFocalLengthAttr(3.06);cam.CreateHorizontalApertureAttr(4.8);cam.CreateVerticalApertureAttr(3.6)
cam.CreateClippingRangeAttr(Gf.Vec2f(.01,10.))

stage.GetRootLayer().Save()
hand_world=UsdGeom.XformCache().GetLocalToWorldTransform(stage.GetPrimAtPath('/World/Franka/panda_hand')).ExtractTranslation()
report={'robot':'NVIDIA Isaac Lab Franka Emika Panda with Panda hand','scene':str(source.relative_to(ROOT)),
        'base_position_m':a.base_position,'joint_positions_rad_or_m':joints,
        'hand_position_m':list(hand_world),'physics_stepped':False,
        'wrist_camera':'Provisional project mount, 4:3 inspection crop, not a calibrated training capture',
        'usd':'pencil_franka.usda','source_scenes_modified':False}
