...
def main():
    ...
    parser.add_argument(
        "--project-root", type=Path, default=ROOT, help="Source/deployment root when checking pulled portable runs"
    )
    parser.add_argument(
        "--in-progress",
        action="store_true",
    ...
    if not catalog.is_absolute():
        catalog = args.project_root / catalog
    ...
    def finite(x):
        ...
        if isinstance(x, dict):
            return sum(finite(v) for v in x.values())
        if isinstance(x, (list, tuple)):
            return sum(finite(v) for v in x)
        return 0
    ...
    assert json.loads(path.with_suffix(".contract.json").read_text()) == json.loads(
        (args.run / "contract.json").read_text()
    )
    ...
    assert losses and all(math.isfinite(v) for values in losses.values() for v in values), (
        "Missing/nonfinite loss metrics"
    )
    report = {
        "passed": True,
        "verification_scope": "in_progress" if args.in_progress else "completed",
        "catalog_sha256": digest,
        "run": str(args.run),
        "checkpoint": str(path),
        "epochs": epoch,
        "transitions": checkpoint["frame"],
        "finite_tensors": count,
        "actor_optimizer_step_min": min(steps),
        "actor_optimizer_step_max": max(steps),
        "losses": losses,
    }
    if cfg.get("multipart_geometry_assignment_version") == 1:
        ...
        with np.load(catalog, allow_pickle=False) as data:
            selected = (
                np.ones(len(data["target_ids"]), dtype=bool)
                if cfg["catalog_split"] == "all"
                else data["split"] == cfg["catalog_split"]
            )
    ...
    if args.in_progress and cfg.get("world_size", 1) > 1:
        ...
        assert all(
            r["completed"] and r["rank"] == i and r["world_size"] == cfg["world_size"] for i, r in enumerate(ranks)
        ), "Incomplete distributed rank report"
    ...
    if cfg.get("resume_checkpoint"):
        ...
        if not baseline_path.is_absolute():
            baseline_path = args.project_root / baseline_path
