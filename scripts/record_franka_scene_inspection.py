...
parser.add_argument(
    "--catalog", type=Path, default=ROOT / "isaac_rl/data/franka_fabrica_plumbers_v2/catalog_training_ready.npz"
)
...
def main():
    ...
    look = [0.48, -0.10, 0.18] if env.lab_contract else [0.35, 0.0, 0.3]
    ...
    selected = [
        "orientation_000__g0544__p0",
        "orientation_000__g1762__p1",
        "orientation_001__g0537__p0",
        "orientation_001__g0533__p1",
        "orientation_002__g3192__p0",
        "orientation_002__g2679__p1",
    ]
    report = {
        "catalog_sha256": hashlib.sha256(args.catalog.read_bytes()).hexdigest(),
        "contract": env.contract(),
        "physics_device": env.device,
        "overview_resolution": [args.overview_width, args.overview_height],
        "fps": 1 / (cfg.sim.dt * cfg.decimation),
        "goal_views": [],
        "episodes": [],
        "notes": [
            "No trained policy used.",
            "Object is kinematic and gripper stays open in alignment task.",
            "RGB gain randomization disabled for inspection.",
            "Camera calibration remains provisional.",
        ],
    }
    if env.appearance:
    ...
    with torch.inference_mode():
        if env.appearance:
            for i in range(12):
                ...
                frame(
                    f"Appearance {i + 1:02d} | {sample['part_color_name']}",
                    f"Light {sample['light_intensity']:.0f} | {sample['light_temperature_k']:.0f} K | canonical goal on right",
        ...
        for mode in ("zero", "scripted"):
            for number, target in enumerate((selected[0], selected[4]), 1):
                ...
                for step in range(env.max_episode_length + 2):
                    label = (
                        "Zero actions (holds initial pose)"
                        if mode == "zero"
                        else "Scripted pose controller (privileged target)"
                    )
                    ...
                    if mode == "scripted":
                        ...
                        for block in (slice(0, 3), slice(3, 6)):
                            action[:, block] /= action[:, block].abs().amax(-1, keepdim=True).clamp_min(1.0)
