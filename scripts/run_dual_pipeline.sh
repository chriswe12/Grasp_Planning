...
usage() {
  cat <<'EOF'
Usage:
  ./run_pipeline.sh --mode sim --robots both [options]
  ./run_pipeline.sh --mode real --robots both [options]

Dual-robot vertical slice using a persistent MoveIt stack by default:
  sim   reuses mock MoveIt, plans a fresh pair, then executes it in Isaac
  real  reuses hardware MoveIt, builds fresh targets, then preflights or executes live

Common options:
  --robots SET                left, right, or both. Default: both
  --single-role ROLE          holder or inserter when one robot is active.
                              Default: inserter
  ...
  --policy NAME_OR_PATH         Real-only policy for every active grasp approach.
  --left-camera NAME            Default: realsense_1
  --right-camera NAME           Default: realsense_2
  --rviz
  --reuse-moveit               Reuse an already-running /lbr_dual_arm stack (default)
  --start-moveit               Start and own a temporary MoveIt stack for this run
  --keep-moveit                With --start-moveit, leave that stack running
...
POLICY=""
LEFT_CAMERA="realsense_1"
RIGHT_CAMERA="realsense_2"
RVIZ=0
REUSE_MOVEIT=1
KEEP_MOVEIT=0
HEADLESS=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --single-role) SINGLE_ROLE="${2:-}"; shift 2 ;;
    ...
    --start-moveit) REUSE_MOVEIT=0; shift ;;
...
if [[ "${ROBOTS}" != "left" && "${ROBOTS}" != "right" && "${ROBOTS}" != "both" ]]; then
  echo "[DUAL-RUN] --robots must be left, right, or both." >&2
  exit 1
fi
if [[ "${SINGLE_ROLE}" != "holder" && "${SINGLE_ROLE}" != "inserter" ]]; then
  echo "[DUAL-RUN] --single-role must be holder or inserter." >&2
  exit 1
fi
if [[ -n "${POLICY}" && "${MODE}" != "real" ]]; then
  echo "[DUAL-RUN] --policy is supported only with --mode real." >&2
  exit 1
fi
if [[ "${ROBOTS}" != "both" && "${MODE}" != "real" ]]; then
  echo "[DUAL-RUN] Single-active-robot task execution is currently real-only." >&2
  exit 1
fi
...
if [[ "${KEEP_MOVEIT}" -eq 1 && "${REUSE_MOVEIT}" -eq 1 ]]; then
  echo "[DUAL-RUN] --keep-moveit is only valid together with --start-moveit." >&2
  exit 1
fi
if [[ -z "${ROBOT_USD}" ]]; then
...
cd "${SCRIPT_DIR}"
source_if_exists "${SCRIPT_DIR}/setup_robot_env.sh"
...
if [[ "${MOVEIT_READY}" -eq 1 ]]; then
  if [[ "${REUSE_MOVEIT}" -ne 1 ]]; then
    ...
    echo "[DUAL-RUN] Stop it before using --start-moveit, or use the default persistent-stack reuse." >&2
    exit 1
  fi
  echo "[DUAL-RUN] Reusing the existing dual MoveIt stack."
else
  if [[ "${REUSE_MOVEIT}" -eq 1 ]]; then
    echo "[DUAL-RUN] No reusable dual MoveIt stack is ready on ROS domain ${ROS_DOMAIN_ID}." >&2
    echo "[DUAL-RUN] Start it separately with --bringup-only, or opt into temporary ownership with --start-moveit." >&2
    echo "[DUAL-RUN] Missing /lbr_dual_arm/compute_ik or /lbr_dual_arm/plan_kinematic_path." >&2
    exit 1
  fi
...
if [[ "${ROBOTS}" != "both" ]]; then
  if [[ "${SINGLE_ROLE}" == "inserter" ]]; then
    INSERTER_ARM="$([[ "${ROBOTS}" == "left" ]] && printf lbr_one || printf lbr_two)"
  else
    INSERTER_ARM="$([[ "${ROBOTS}" == "left" ]] && printf lbr_two || printf lbr_one)"
  fi
  TASK_BUILD_ARGS=(
    "${COMMON_TASK_ARGS[@]}"
    --inserter-arm "${INSERTER_ARM}"
...
if [[ "${ROBOTS}" == "both" ]]; then
  REAL_ARGS+=(--active-roles holder,inserter)
else
  REAL_ARGS+=(--active-roles "${SINGLE_ROLE}")
fi
if [[ -n "${POLICY}" ]]; then
  REAL_ARGS+=(
    --policy "${POLICY}"
