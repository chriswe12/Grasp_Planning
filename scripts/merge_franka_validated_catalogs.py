...
TARGET_KEYS=('target_ids','split','validated','joint_paths','goal_rgbd','goal_poses','object_poses',
             'open_widths','jaw_widths','source_grasp_ids','orientation_ids','target_part_indices',
             'part_keys','lab_approach_validated','lift_validated')


def read(path, stage):
    with np.load(path,allow_pickle=False) as source:
        result={key:source[key].copy() for key in source.files}
    for key in (('validated','lab_approach_validated','lift_validated') if stage=='lift' else ('validated','lab_approach_validated')):
...
def main():
    ...
    base=read(args.base,args.stage);extra=read(args.additional,args.stage)
    contract=json.loads(str(base['contract_json'].item()))
    extra_contract=json.loads(str(extra['contract_json'].item()))
    left=deepcopy(contract);right=deepcopy(extra_contract)
    assets=left.pop('object_assets');extra_assets=right.pop('object_assets')
    ...
    for index in np.unique(extra['target_part_indices']):
        item=extra_assets[int(index)];part=item['part_key']
        existing=next(i for i,a in enumerate(assets) if a['part_key']==part)
        old_mesh=mesh_for(part,base['source_bundle_paths']);new_mesh=mesh_for(part,extra['source_bundle_paths'])
        assert np.array_equal(old_mesh.faces,new_mesh.faces),'Mesh topology changed'
        assert np.allclose(old_mesh.vertices_obj,new_mesh.vertices_obj,rtol=0.,atol=1e-9),'Bundle-local geometry/frame changed'
        assert np.isclose(assets[existing]['object_mass_kg'],item['object_mass_kg'])
        assets[existing]=item;mapping[int(index)]=existing
    ...
    for data in (base,extra):
        for path,digest in zip(data['source_bundle_paths'],data['source_bundle_sha256'],strict=True):
            if path in sources:assert sources[path]==digest,'Source file identity changed'
    ...
    for target,part_index in zip(result['target_ids'],result['target_part_indices']):
        row=deepcopy(lookup[str(target)]);row['part_index']=int(part_index);targets.append(row)
    ...
    for path,data in ((args.base,base),(args.additional,extra)):
        for target in data['target_ids']:
            ...
            if image.is_file():shutil.copyfile(image,args.output.parent/image.name)
    manifest=dict(schema_version=3,targets=targets,object_assets=assets,
        sources=[dict(path=path,sha256=digest) for path,digest in sources.items()],
        inputs=[dict(path=str(p),sha256=hashlib.sha256(p.read_bytes()).hexdigest()) for p in (args.base,args.additional)],
        validation_stage=args.stage,
        addition_policy=('Previously missing parts only; same bundle-local mesh and physical mass; validated labels retained' if args.stage=='lift' else 'Approach-only merge; physical labels cleared for a new complete lift validation'))
    (args.output.parent/'source_manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
    temporary=args.output.with_suffix('.tmp.npz');np.savez_compressed(temporary,**result);temporary.replace(args.output)
    print(json.dumps(dict(output=str(args.output),targets=len(result['target_ids']),parts=len(set(result['part_keys'])),added_parts=sorted(set(extra['part_keys'].tolist()))),indent=2))
