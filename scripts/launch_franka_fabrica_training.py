...
def main():
    ...
    parser.add_argument(
        "--catalog", type=Path, default=ROOT / "isaac_rl/data/franka_fabrica_pencil_randomized/catalog.npz"
    )
    parser.add_argument(
        "--controller-report", type=Path, default=ROOT / "artifacts/franka_randomized/controller_check.json"
    )
    parser.add_argument(
        "--smoke-report", type=Path, default=ROOT / "artifacts/franka_scaling/resume_probe.verification.json"
    )
    parser.add_argument(
        "--appearance-report", type=Path, default=ROOT / "artifacts/franka_scaling/batched_appearance_check.json"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Environments per GPU; 32 was fastest in the local pencil-lab benchmark",
    )
    ...
    gpu_ids = (
        subprocess.check_output(["nvidia-smi", "--query-gpu=uuid", "--format=csv,noheader"], text=True)
        .strip()
        .splitlines()
    )
    gpu_count = args.gpu_count or len(gpu_ids)
    if not 1 <= gpu_count <= len(gpu_ids):
        parser.error(f"Only {len(gpu_ids)} local GPUs available")
    ...
    existing = subprocess.check_output(
        ["docker", "ps", "-q", "--filter", "label=codex.task=franka-fabrica-zed-training"], text=True
    ).strip()
    if existing:
    ...
    command = [
        ...
        name,
        "--label",
        "codex.task=franka-fabrica-zed-training",
        "--gpus",
        str(gpu_count),
        "--network",
        "host",
        ...
        "-e",
        f"FRANKA_CUDNN_MODE={args.cudnn_mode}",
        "-v",
    ...
    if cache.exists():
        command += ["-v", f"{cache}:/root/.cache/torch/hub/checkpoints:ro"]
    command += [
        "--entrypoint",
        "/bin/bash",
        "isaac-lab-euler:2.3.2",
        "/workspace/project/scripts/run_franka_fabrica_training.sh",
        "/workspace/project/" + relative.as_posix(),
        inside,
        str(args.num_envs),
        str(args.iterations),
        str(gpu_count),
    ...
    launch = {
        ...
        "gpu_count": gpu_count,
        "total_envs": args.num_envs * gpu_count,
        ...
        "automatic_post_training_evaluation": ["validation", "test"],
    }
