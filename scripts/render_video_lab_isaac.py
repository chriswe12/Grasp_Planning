...
parser.add_argument('--prepared-scene',type=Path,help='Render all cameras in an already composed USD, preserving its lights.')
args=parser.parse_args()
if min(args.width,args.height,args.warmup_frames,args.subframes)<1: parser.error('Sizes and frame counts must be positive.')
...
app=SimulationApp({'headless':True,'renderer':'RaytracedLighting','width':args.width,'height':args.height,
    'active_gpu':0,'physics_gpu':0,'multi_gpu':False,'sync_loads':True,
    'disable_viewport_updates':True,'create_new_stage':False,
    'extra_args':['--/app/file/ignoreUnsavedOnExit=true','--/app/asyncRendering=false','--/omni/replicator/asyncRendering=false']})
...
def configure_renderer():
    ...
    for k,v in settings.items(): carb.settings.get_settings().set(k,v)
    return settings
...
def setup_rl_lighting(stage):
    ...
    dome.CreateIntensityAttr(450.); dome.CreateColorAttr(Gf.Vec3f(.76,.80,.86))
    key=UsdLux.DistantLight.Define(stage,'/World/RenderKey')
    key.CreateIntensityAttr(1200.); key.CreateColorAttr(Gf.Vec3f(1.,.91,.82)); key.CreateAngleAttr(8.)
    key.AddOrientOp().Set(Gf.Quatf(.9384303340467282,Gf.Vec3f(.1620103355210184,-.2851488516085399,-.10858771455208008)))
...
def capture(stage,camera,output):
    ...
    if not np.isfinite(distances).any(): raise RuntimeError('No finite camera depth')
    Image.fromarray(pixels[:,:,:3]).save(output.with_suffix('.png'))
    np.save(str(output)+'_depth_m.npy',distances)
    cam=UsdGeom.Camera(camera)
    record={'rgb':str(output.with_suffix('.png').relative_to(args.output_dir)),
        'depth':str(Path(str(output)+'_depth_m.npy').relative_to(args.output_dir)),
        'camera_prim':str(camera.GetPath()),'resolution':[args.width,args.height],
...
def main():
    settings=configure_renderer(); report={'renderer':'Isaac Sim 5.1 RTX RaytracedLighting','antialiasing':'DLAA',
        'lighting':args.lighting,'render_settings':settings,'physics_stepped':False,
        'note':'Inspection cameras, not calibrated robot-mounted sensors. No RGB/depth augmentation. Source assets unchanged.',
        'captures':[], 'curve_repairs':{}}
    ...
    for variant in args.variants:
        ...
        target=args.output_dir/variant; target.mkdir(exist_ok=True)
        ...
        for with_props in modes:
            ...
            if args.lighting=='rl': setup_rl_lighting(composed)
            ...
            if not with_props: composed.GetPrimAtPath('/World/Props').SetActive(False)
            ...
            for prim in stage.Traverse():
                if prim.IsA(UsdGeom.Camera):
                    for name,needle in [('overview','Camera_Overview'),('detail','Camera_Table_Detail')]:
                        if needle in str(prim.GetPath()): cameras[name]=prim
            if len(cameras)!=2: raise RuntimeError(f'Missing exported cameras: {cameras}')
            for name,camera in cameras.items():
                ...
                (args.output_dir/'render_report.json').write_text(json.dumps(report,indent=2)+'\n')
        assert hashes=={str(p.relative_to(folder)):hashlib.sha256(p.read_bytes()).hexdigest() for p in folder.rglob('*') if p.is_file()}
    report['status']='complete'; report['sources_unchanged']=True
    (args.output_dir/'render_report.json').write_text(json.dumps(report,indent=2)+'\n')
    print('ISAAC_VIDEO_LAB_RENDERS_COMPLETE',len(report['captures']),flush=True)
