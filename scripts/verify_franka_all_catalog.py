...
def audit(catalog, root=ROOT):
    ...
    for key in ('joint_paths', 'goal_rgbd', 'goal_poses', 'object_poses', 'open_widths', 'jaw_widths',
                'target_part_indices', 'part_keys', 'split', 'source_grasp_ids', 'orientation_ids',
    ...
    for key in ('goal_poses', 'object_poses'):
        assert np.allclose(np.linalg.norm(data[key][:, 3:], axis=1), 1., atol=1e-4), key
    assert (data['open_widths'] <= .080001).all()
    assert (data['open_widths'] >= data['jaw_widths'] + .0079).all()
    ...
    split_report = {split: dict(targets=int((data['split'] == split).sum()),
    ...
    return dict(passed=True, catalog=str(catalog),
                catalog_sha256=hashlib.sha256(Path(catalog).read_bytes()).hexdigest(),
                target_count=count, source_bundles=len(sources), usable_parts=len(observed),
                lift_protocol=json.loads(str(data['lift_validation_json'].item())) if 'lift_validation_json' in data else 'legacy_root_height_v1',
                expected_parts=len(expected), excluded_parts=sorted(expected-observed),
                per_part_targets=dict(sorted(Counter(data['part_keys'].tolist()).items())),
                splits=split_report, checks=['finite_arrays', 'matched_geometry', 'unique_targets',
                'source_and_asset_sha256', 'grouped_split_isolation', 'approach_and_dynamic_lift_labels'])
