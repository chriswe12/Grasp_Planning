...
for episode in report['episodes']:
    ...
    video = out / (name + '.mp4')
    subprocess.run([args.ffmpeg, '-hide_banner', '-loglevel', 'error', '-y', '-framerate', str(report['fps']),
                    '-i', str(out / name / '%04d.png'), '-c:v', 'libx264', '-crf', '19', '-pix_fmt', 'yuv420p',
    ...
    cards.append(f'<article><h3>{title} · {html.escape(episode["target_id"])}</h3>'
...
gallery = ''.join(f'<a href="{g["image"]}"><img loading="lazy" src="{g["image"]}" alt="{html.escape(g["target_id"])}"></a>' for g in goals)
...
if variations:
    ...
    subprocess.run([args.ffmpeg, '-hide_banner', '-loglevel', 'error', '-y', '-framerate', '0.5',
                    '-i', str(out / 'variation_%02d.png'), '-frames:v', str(len(variations)*30),
                    '-r', '15', '-c:v', 'libx264', '-crf', '19', '-pix_fmt', 'yuv420p',
                    '-movflags', '+faststart', str(sweep)], check=True)
    ...
    variation_gallery += ''.join(f'<a href="{v["image"]}"><img src="{v["image"]}" loading="lazy" alt="Appearance sample"></a>' for v in variations)
