...
def main():
    ...
    parser.add_argument('--catalog', type=Path, default=Path('isaac_rl/data/franka_fabrica_pencil_randomized/catalog.npz'))
    parser.add_argument('--copy-sources-from', type=Path,
                        help='Stage the exact catalog-referenced grasp bundles before verification')
    ...
    with np.load(catalog, allow_pickle=False) as source:
        ...
        split_counts = {split: int((source['split'] == split).sum())
                        for split in ('train', 'validation', 'test')}
        sources = list(zip(source['source_bundle_paths'].tolist(), source['source_bundle_sha256'].tolist(), strict=True))
    ...
    report = dict(passed=True, root=str(root), catalog_sha256=hashlib.sha256(catalog.read_bytes()).hexdigest(),
                  splits=split_counts, source_bundles=len(sources), file_count=len(files), total_bytes=sum(f['bytes'] for f in files.values()),
                  files=files, scope='Portable source/data/asset checks only; no simulator or optimizer execution')
