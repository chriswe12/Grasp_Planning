...
def main():
    ...
    parser.add_argument('--initial-job',type=int,required=True,help='Successful production-catalog PPO gate; never a diagnostic dataset')
    ...
    parser.add_argument('--startup-epochs',type=int,default=0,
    ...
    assert hashlib.sha256((ROOT/args.catalog).read_bytes()).hexdigest()==audit['catalog_sha256'], 'Catalog changed after its audit'
    plan=segment_plan(args.segments,args.epochs_per_segment,args.time_limit,args.startup_epochs,args.startup_time_limit)
    ...
    login=subprocess.check_output(['bash','-c','source "$1"; printf "%s" "$EULER_LOGIN"',
                                   'bash',str(args.config.resolve())],text=True).strip()
    gate_record=subprocess.check_output(['ssh',login,'sacct','-X','-j',str(args.initial_job),
        '--noheader','--parsable2','--format=State,ExitCode'],text=True,timeout=60).strip().splitlines()
    ...
    report=dict(catalog=args.catalog,catalog_sha256=audit['catalog_sha256'],
        config=str(args.config.resolve()),initial_job=args.initial_job,jobs=[],
        initial_gate_state=gate_state,initial_gate_exit_code=gate_exit,
        gpu_count=4,environments_per_gpu=32,global_rollout_frames=8192,
    ...
    for segment,final_epoch,time_limit in plan:
        command=['bash',str(ROOT/'euler/submit.sh'),'franka-train','--gpu-count','4',
            '--global-minibatch-size','1024','--num-envs','32','--catalog',args.catalog,
            '--iterations',str(final_epoch),'--save-frequency','100',
            '--resume-job',str(previous),'--evaluate-after',
            '--job-label',f'franka-all-long-s{segment}','--time-limit',time_limit]
        ...
        if segment==args.segments:command.append('--evaluate-test-after')
        result=subprocess.run(command,cwd=ROOT,env=env,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        print(result.stdout,flush=True)
        match=re.search(r'Submitted batch job (\d+)',result.stdout)
        if result.returncode or not match:raise RuntimeError('Submission failed; completed submissions remain recorded in manifest')
        job=int(match.group(1));report['jobs'].append(dict(segment=segment,job_id=job,predecessor=previous,
            final_epoch=final_epoch,time_limit=time_limit,command=command));save();previous=job
    log=args.output.with_suffix('.watch.log')
    with log.open('a') as output:
        process=subprocess.Popen([sys.executable,str(ROOT/'euler/watch_franka_chain.py'),str(args.output)],
            cwd=ROOT,env=env,stdin=subprocess.DEVNULL,stdout=output,stderr=subprocess.STDOUT,
            start_new_session=True,close_fds=True)
    report['watcher_pid']=process.pid;report['watcher_log']=str(log);save()
