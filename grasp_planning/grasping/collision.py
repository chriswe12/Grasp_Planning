...
def _load_pdz_gripper_mesh(key: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    mesh_names = {
        ...
        "left_finger": "left_finger.stl",
        "right_finger": "right_finger.stl",
        "left_pad": "left_pad_8mm.stl",
        "right_pad": "right_pad_8mm.stl",
    }
    mesh_path = _PDZ_GRIPPER_MESH_DIR / mesh_names[key]
    if not mesh_path.is_file():
        raise FileNotFoundError(f"PDZ gripper collision mesh not found at '{mesh_path}'.")
    mesh = trimesh.load(mesh_path, force="mesh")
    result = (
        np.asarray(mesh.vertices, dtype=float) * 0.001,
...
class PdzGripperCollisionModel:
    """Conservative hand and finger/pad hulls for the PDZ gripper."""
    ...
    def _components(
        ...
        *,
    ...
    ) -> tuple[tuple[tuple[str, str, np.ndarray], ...], np.ndarray, np.ndarray]:
        rotmat = np.asarray(grasp_rotmat, dtype=float)
        contact_a = np.asarray(contact_point_a, dtype=float)
        contact_b = np.asarray(contact_point_b, dtype=float)
        nominal_center = (
        ...
        base_origin = tcp_center - body_rotmat @ _PDZ_GRIPPER_BASE_TO_GRASP_CENTER_M
        opening_m = float(np.linalg.norm(contact_b - contact_a)) + 2.0 * float(
            self.contact_gap_m
        )
        travel = max(0.0, 0.5 * (opening_m - _PDZ_GRIPPER_CLOSED_PAD_GAP_M))
        if travel > 0.032 + 1.0e-9:
            raise ValueError(
                f"PDZ jaw opening {opening_m:.4f} m exceeds the 0.076 m maximum."
            )
        components = (
            ("pdz_gripper_base", "base", np.zeros(3)),
            ("pdz_left_finger", "left_finger", np.array([-travel, 0.0, 0.0])),
            ("pdz_right_finger", "right_finger", np.array([travel, 0.0, 0.0])),
        )
        return components, base_origin, body_rotmat
    ...
    def primitives_for_grasp(
        ...
        *,
        ...
        components, base_origin, body_rotmat = self._components(
            grasp_rotmat=grasp_rotmat,
            contact_point_a=contact_point_a,
            contact_point_b=contact_point_b,
            grasp_center=grasp_center,
        ...
        for name, key, translation in components:
            ...
            transformed = (
                (vertices + translation[None, :]) @ body_rotmat.T
                + base_origin[None, :]
            )
            primitives.append(
                MeshCollisionPrimitive(
                    name=name, vertices_obj=transformed, faces=faces
                )
            )
        return tuple(primitives)
    ...
    def world_component_aabb_bounds_for_grasp(
        ...
        *,
        ...
        components, base_origin, body_rotmat_obj = self._components(
        ...
        rotation_world = np.asarray(rotation_world_from_object, dtype=float)
        translation_world = np.asarray(translation_world_from_object, dtype=float)
        body_rotmat_world = rotation_world @ body_rotmat_obj
        base_origin_world = rotation_world @ base_origin + translation_world
        bounds = []
        for name, key, translation in components:
            vertices, _ = _load_pdz_gripper_collision_hull(key)
            transformed = (
                (vertices + translation[None, :]) @ body_rotmat_world.T
                + base_origin_world[None, :]
            )
