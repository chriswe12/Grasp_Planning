...
PLANNER_HAND_TO_CONTACT_M = 0.0584 + 0.04525
...
def task_pose(grasp, object_pose, tcp_offset=(0.0, 0.0, 0.1034)):
    """Use the shared pad-offset-corrected transform, then match task TCP."""
    world = saved_grasp_to_world_grasp(grasp, object_pose, pregrasp_offset=0.065, gripper_width_clearance=0.008)
...
def collect_targets(benchmark_root, *, per_orientation=24, tcp_offset=(0.0, 0.0, 0.1034), part_key=None):
    ...
    for path in paths:
        ...
        candidates = [c for c in candidates if 0.002 <= c.jaw_width <= 0.072]
        chosen = []
        for c in candidates:
            rc = quat_to_rotmat_xyzw(c.grasp_orientation_xyzw_obj)
            if all(
                np.linalg.norm(np.asarray(c.grasp_position_obj) - p.grasp_position_obj) > 0.004
                or np.linalg.norm(rc - quat_to_rotmat_xyzw(p.grasp_orientation_xyzw_obj)) > 0.35
                for p in chosen
            ):
                chosen.append(c)
        ...
        for c in chosen:
            ...
            for placement, (px, py, yaw) in enumerate(((0.43, -0.035, -0.18), (0.48, 0.035, 0.18))):
                ...
                targets.append(
                    {
                        "target_id": f"{path.parent.name}__{c.grasp_id}__p{placement}",
                        "source_bundle": source_id,
                        "source_grasp_id": c.grasp_id,
                        "orientation_id": path.parent.name,
                        "split": split,
                        "jaw_width_m": c.jaw_width,
                        "open_width_m": c.jaw_width + 0.008,
                        "object_pose": [px, py, pz, quat[3], *quat[:3]],
                        "goal_pose": goal.tolist(),
                        "approach_axis": approach.tolist(),
                        "score": c.score,
                    }
                )
    if part_key is not None:
    ...
    return {
        ...
        "first_bundle": portable_path(first_bundle),
        "targets": targets,
        "planner_hand_to_contact_m": PLANNER_HAND_TO_CONTACT_M,
        "task_tcp_offset_m": list(tcp_offset),
    }, mesh
