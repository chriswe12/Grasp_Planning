...
def sample_prop_layout(seed: int, manifest: dict) -> list[dict]:
    ...
    """
    rng=random.Random(seed); cfg=manifest['randomization']; t=manifest['dimensions']
    ...
    placed=[]
    for prop in chosen:
        radius=prop['footprint_radius_m']; w=t['width']; d=t['depth']
        if 2*(radius+.025)>=min(w,d): continue
        for _ in range(300):
            x=rng.uniform(-w/2+radius+.025,w/2-radius-.025)
            y=rng.uniform(-d/2+radius+.025,d/2-radius-.025)
            if any(math.hypot(x-a,y-b)<radius+r+.02 for a,b,r in reserved+[(p['position'][0],p['position'][1],p['radius']) for p in placed]): continue
            placed.append(dict(name=prop['name'],asset=prop['asset'],position=[x,y,.0004],yaw_rad=rng.uniform(-math.pi,math.pi),radius=radius)); break
    return placed
...
def add_environment(stage, prim_path: str, asset_dir=DEFAULT_ASSET_DIR):
    ...
    if stage.GetPrimAtPath(prim_path): raise ValueError(f'Prim already exists: {prim_path}')
    root=UsdGeom.Xform.Define(stage,prim_path).GetPrim()
    root.GetReferences().AddReference(str(asset)); root.SetInstanceable(False)
...
def repair_exported_curve_widths(stage, prim_path: str) -> list[dict]:
    ...
    for prim in list(Usd.PrimRange(root)):
        if not prim.IsA(UsdGeom.BasisCurves):
            continue
        name = prim.GetName()
        diameter = next((d for prefix, d in (
            ('Grommet_slot', .0013), ('Wall_power_lead', .006),
            ('Loose_floor_cable', .006)) if name.startswith(prefix)), None)
        if diameter is None:
            continue
        ...
        if parent.GetName() == name + '_0' and original.IsA(UsdGeom.BasisCurves):
            ...
            if (other.GetPointsAttr().Get() == points
                    and cache.GetLocalToWorldTransform(original) == cache.GetLocalToWorldTransform(prim)):
        ...
        changes.append(dict(prim=str(prim.GetPath()), action='restore_bevel_diameter',
                            previous_widths=previous, diameter_m=diameter))
    return changes
...
def randomize_environment(stage, prim_path: str, seed: int, asset_dir=DEFAULT_ASSET_DIR):
    ...
    manifest=json.loads((Path(asset_dir)/'manifest.json').read_text()); cfg=manifest['randomization']
    rng=random.Random(seed); gain=rng.uniform(*cfg['table_color_gain']); rough=rng.uniform(*cfg['table_roughness']); wg=rng.uniform(*cfg['wall_color_gain']); wear=rng.random()<cfg['wear_visibility_probability']
    root=stage.GetPrimAtPath(prim_path)
    if not root: raise ValueError(f'Missing environment prim: {prim_path}')
    changes=0
    for prim in Usd.PrimRange(root):
        ...
        if prim.IsA(UsdShade.Shader):
            ...
            if '/Table_Laminate/' in path:
                if shader.GetIdAttr().Get()=='UsdUVTexture' and prim.GetName()=='Laminate_Albedo':
                    shader.CreateInput('scale',Sdf.ValueTypeNames.Float4).Set(Gf.Vec4f(gain,gain,gain,1)); changes+=1
                if shader.GetIdAttr().Get()=='UsdPreviewSurface':
                    inp=shader.GetInput('roughness'); inp.DisconnectSource(); inp.Set(rough); changes+=1
            if '/Warm_White_Partition/' in path and shader.GetIdAttr().Get()=='UsdPreviewSurface':
                ...
                if not attr: attr=prim.CreateAttribute('lab:nominalDiffuseColor',Sdf.ValueTypeNames.Color3f,custom=True); attr.Set(inp.Get())
        ...
        if prim.GetName().startswith(('Tape_Remnant','Handling_Scuff','Faded_fixture_outline')):
            ...
            if img: img.CreateVisibilityAttr().Set('inherited' if wear else 'invisible')
    if changes!=2: raise RuntimeError(f'Expected tabletop color and roughness inputs, found {changes}')
    result=dict(seed=seed,table_color_gain=gain,table_roughness=rough,wall_color_gain=wg,wear_visible=wear)
    root.CreateAttribute('lab:appearanceSample',Sdf.ValueTypeNames.String,custom=True).Set(json.dumps(result,sort_keys=True))
    return result
...
def add_mock_props(stage, prim_path: str, seed: int, asset_dir=DEFAULT_ASSET_DIR):
    ...
    folder=Path(asset_dir).resolve(); manifest=json.loads((folder/'manifest.json').read_text())
    if stage.GetPrimAtPath(prim_path): raise ValueError(f'Prim already exists: {prim_path}')
