...
def validate_profile(profile):
    if profile['profile'] != 'franka_pencil_episode_appearance_v1' or profile['goal_appearance'] != 'canonical_saved_reference':
    ...
    for key in ('canonical_part_fraction', 'wear_visible_fraction'):
        if not 0 <= profile[key] <= 1: raise ValueError(key)
...
def sample_appearance(profile, seed):
    ...
    draw = lambda name: rng.uniform(*profile[name])
    ...
    sample = dict(seed=int(seed), part_color_name=name,
        part_color=[min(.95, x*gain) for x in profile['palette'][name]],
        part_roughness=draw('part_roughness'),
        table_scale=[table*draw('table_channel_gain') for _ in range(3)],
        table_roughness=draw('table_roughness'), wall_gain=draw('wall_gain'),
        wear_visible=rng.random() < profile['wear_visible_fraction'],
        light_intensity=draw('light_intensity'), light_radius_m=draw('light_radius_m'),
        light_position=[draw('light_x_m'), draw('light_y_m'), draw('light_z_m')],
        light_temperature_k=draw('light_temperature_k'),
        exposure_ev=draw('exposure_ev'), white_balance=[draw('white_balance_gain') for _ in range(3)],
...
class FrankaAppearanceRandomizer:
    ...
    def __init__(self, stage, roots, origins, prop_names, profile):
        ...
        for root in roots:
            def shaders(path):
                return [UsdShade.Shader(p) for p in Usd.PrimRange(stage.GetPrimAtPath(path))
            ...
            if len(part) != 1: raise RuntimeError(f'Expected one bound part shader at {root}: {part}')
            ...
            if len(albedo) != 1 or len(surface) != 1: raise RuntimeError('Missing laminate shader inputs')
            surface[0].GetInput('roughness').DisconnectSource()
            walls = [(s.GetInput('diffuseColor'), tuple(s.GetInput('diffuseColor').Get())) for s in lab
                     if '/Warm_White_Partition/' in str(s.GetPath()) and s.GetIdAttr().Get() == 'UsdPreviewSurface']
            wear = [UsdGeom.Imageable(p).CreateVisibilityAttr() for p in Usd.PrimRange(stage.GetPrimAtPath(root+'/Lab'))
                    if p.GetName().startswith(('Tape_Remnant', 'Handling_Scuff', 'Faded_fixture_outline'))]
            props = [[s for s in shaders(root+'/'+name) if s.GetIdAttr().Get() == 'UsdPreviewSurface'] for name in prop_names]
            ...
            move = next(op for op in UsdGeom.Xformable(key_prim).GetOrderedXformOps() if op.GetOpType() == UsdGeom.XformOp.TypeTranslate)
            self.handles.append(dict(part=part[0], table=surface[0], albedo=albedo[0].CreateInput('scale', Sdf.ValueTypeNames.Float4), walls=walls, wear=wear,
                                     props=props, light=light, sphere=UsdLux.SphereLight(key_prim), move=move))
    ...
    def _spec(self, attribute):
        ...
        if path not in self._specs:
            ...
            if spec is None:
                ...
                spec = Sdf.AttributeSpec(owner, attribute.GetName(), attribute.GetTypeName(),
                                         attribute.GetVariability(), attribute.IsCustom())
    ...
    def _edits(self, index, sample):
        ...
        put(h['albedo'].GetAttr(), Gf.Vec4f(*sample['table_scale'], 1.))
        put(h['table'].GetInput('roughness').GetAttr(), sample['table_roughness'])
        for attr, original in h['walls']:
            put(attr.GetAttr(), Gf.Vec3f(*[min(.98, c*sample['wall_gain']) for c in original]))
        for attr in h['wear']:
