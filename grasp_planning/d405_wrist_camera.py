...
D405_VISUAL_SERVO_CAMERA_PROFILE = (
    f"d405_color_848x480_intrinsics_260322275185_render_256x144_{D405_PDZ_NAMED_FRAME_MOUNT_PROFILE}"
)
D405_VISUAL_SERVO_OBSERVATION_PROFILE = "rgbd_render_256x144_valid_area_128x72_d405_range_v3"

# Audited named-frame pose from the PDZ Slim gripper CAD/URDF. The complete
# pose is link7 -> PDZ base (+35 mm) -> D405 color optical frame. Real visual
# servoing only needs its rotation to express camera-frame TCP commands in the
# robot base while the full PDZ robot description is being integrated.
D405_PDZ_CAMERA_OPTICAL_POSITION_IN_LINK7_M = (0.009, -0.050560254038, 0.097927071163)
D405_PDZ_CAMERA_OPTICAL_RPY_IN_LINK7_RAD = (math.pi / 6.0, 0.0, math.pi)
...
class D405WristCameraConfig:
    ...
    # New checkpoints use the named camera frame authored in the physical PDZ
    # gripper model. The legacy calibration remains selectable for old assets.
...
def camera_mount_profile_from_camera_profile(camera_profile: str) -> str:
    """Resolve the authored mount encoded by a deployment camera profile."""

    profile = str(camera_profile).strip()
    if profile.endswith(D405_PDZ_NAMED_FRAME_MOUNT_PROFILE):
        return D405_PDZ_NAMED_FRAME_MOUNT_PROFILE
    if profile.endswith("hand_eye_link7_ee35mm_z180_v7"):
        return D405_LEGACY_HAND_EYE_MOUNT_PROFILE
    raise ValueError(f"Unsupported D405 camera profile '{camera_profile}'.")
...
def camera_pose_in_link7(config: D405WristCameraConfig) -> tuple[tuple[float, ...], tuple[float, ...]]:
    ...
    if config.mount_profile == D405_PDZ_NAMED_FRAME_MOUNT_PROFILE:
        position = D405_PDZ_CAMERA_OPTICAL_POSITION_IN_LINK7_M
        rotation = pdz_camera_rotation_in_link7()
...
def camera_rotation_in_link7(config: D405WristCameraConfig) -> np.ndarray:
    ...
    if config.mount_profile == D405_PDZ_NAMED_FRAME_MOUNT_PROFILE:
        return pdz_camera_rotation_in_link7()
    if config.mount_profile != D405_LEGACY_HAND_EYE_MOUNT_PROFILE:
        raise ValueError(f"Unsupported D405 mount profile '{config.mount_profile}'.")
...
def pdz_camera_rotation_in_link7() -> np.ndarray:
    """Return the audited physical PDZ D405 optical rotation in link7."""

    roll, pitch, yaw = D405_PDZ_CAMERA_OPTICAL_RPY_IN_LINK7_RAD
