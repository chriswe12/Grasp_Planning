...
function pairScore(pair){const C=data.scoring,T=targets(pair),H=armScore(T.holder,state.layout.holder_base),I=armScore(T.ins,state.layout.inserter_base),reach=(C.target_min_weight*Math.min(H.score,I.score)+C.target_mean_weight*.5*(H.score+I.score))/(C.target_min_weight+C.target_mean_weight),hs=shoulder(state.layout.holder_base),is=shoulder(state.layout.inserter_base),hd=Math.hypot(...sub(T.hp.position,hs)),ho=Math.hypot(...sub(T.hp.position,is)),id=Math.hypot(...sub(T.pre.position,is)),io=Math.hypot(...sub(T.pre.position,hs)),ownH=clamp(.5+(ho-hd)/(2*C.ownership_margin_m)),ownI=clamp(.5+(io-id)/(2*C.ownership_margin_m)),own=(C.target_min_weight*Math.min(ownH,ownI)+C.target_mean_weight*.5*(ownH+ownI))/(C.target_min_weight+C.target_mean_weight),cross=cross2(hs,T.hp.position,is,T.pre.position),non=cross?0:1,layout=(C.ownership_weight*own+C.noncrossing_weight*non)/(C.ownership_weight+C.noncrossing_weight),tw=C.offline_pair_weight+C.reachability_weight+C.layout_weight,before=tw>0?(C.offline_pair_weight*clamp(pair.offline_score)+C.reachability_weight*reach+C.layout_weight*layout)/tw:0,penalty=cross?(C.crossing_penalty||0):0,total=clamp(before-penalty);return{...pair,score:total,score_before_crossing_penalty:before,crossing_penalty_applied:penalty,holder:H,inserter:I,reachability_score:reach,layout_score:layout,ownership_score:own,holder_ownership_score:ownH,inserter_ownership_score:ownI,segments_cross_xy:cross,targets:T}}
