...
<span>pickup floor</span><span id="pickup-counts">-</span><span>Stage-3 pool</span><span id="stage3-counts">-</span><span>pose fallback</span><span id="fallback-counts">-</span><span>pose queue</span><span id="queue-counts">-</span><span>joint pre-rank</span><span id="joint-counts">-</span><span>exact IK</span><span id="ik-counts">-</span></div></div>
...
<script>
const $=id=>document.getElementById(id),canvas=$("scene"),ctx=canvas.getContext("2d");let live=null,scene=null,lastRevision=-1,lastStateRevision=-1,lastVisualKey="",renderQueued=false,frameBounds={c:[.55,0,.18],e:1};
...
function stage(){const p=live?.phase||"";if(p.startsWith("holder_"))return"holder";if(p==="inserter_pickup_pregrasp"||p==="inserter_pickup_grasp")return"inserter";if(p==="joint_space_ranking"||p.startsWith("inserter_")||p==="transition")return"transition";return p==="complete"?"transition":""}
