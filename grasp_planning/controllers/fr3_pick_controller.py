...
class FR3PickController:
    ...
    def __init__(
        ...
        *,
        ...
        self._ee_to_tcp_offset = self._resolve_ee_to_tcp_offset(self._ee_body_name)
    ...
    def _configure_tcp_approach_targets(
        ...
        grasp_tcp_position_w, grasp_tcp_orientation_xyzw = grasp_pose_to_tcp_pose(
            position_w=self._grasp.position_w,
            orientation_xyzw=self._grasp.orientation_xyzw,
        ...
        self._grasp_tcp_position_w = (
            ...
            max(float(grasp_tcp_position_w[2]), self._MIN_TCP_Z_M),
        )
        self._grasp_tcp_quat_w = quat_xyzw_to_wxyz(
            torch.tensor([grasp_tcp_orientation_xyzw], dtype=torch.float32, device=self._device)
        )
    ...
    def _current_tcp_pose_w(self) -> tuple[tuple[float, float, float], torch.Tensor]:
        ...
        tcp_pos_w = self._ee_position_to_tcp_position_w(ee_pose_w[:, :3], ee_quat_w)
        return tuple(float(v) for v in tcp_pos_w[0].tolist()), ee_quat_w
    ...
    def _ee_to_tcp_offset_tensor(self) -> torch.Tensor:
        offset = self._ee_to_tcp_offset
        if offset is None:
            return torch.zeros((1, 3), dtype=torch.float32, device=self._device)
        return torch.tensor([offset], dtype=torch.float32, device=self._device)

    def _ee_position_to_tcp_position_w(self, ee_position_w: torch.Tensor, ee_quat_w: torch.Tensor) -> torch.Tensor:
        if self._ee_to_tcp_offset is None:
            return ee_position_w
        from isaaclab.utils.math import quat_apply

        return ee_position_w + quat_apply(ee_quat_w, self._ee_to_tcp_offset_tensor())
